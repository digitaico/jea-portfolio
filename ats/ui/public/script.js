...
document
  ...
  .addEventListener("submit", async (event) => {
    ...
    formData.append("domain", "software_development");

    // Make sure this URL matches your Flask backend address and port
    const backendUrl = "http://127.0.0.1:5000/compare_cv";
    ...
    try {
      ...
      let resultHtml = `
            ...
            <p><strong>Prioritized Skill Match:</strong> ${
              scores.prioritized_skill_score * 100
            }%</p>
            <p><strong>Combined Score:</strong> ${
              scores.combined_score * 100
            }%</p>
        `;

      // Display Matched Items (categorized, with section info)
      if (
        scores.matched_items &&
        Object.keys(scores.matched_items).length > 0
      ) {
        ...
        for (const label in scores.matched_items) {
          if (scores.matched_items[label].length > 0) {
            ...
            scores.matched_items[label].forEach((item_info) => {
              ...
              resultHtml += `<li>'${
                item_info.text
              }' (in sections: ${item_info.matched_in_sections.join(
                ", "
              )})</li>`;
            });
            resultHtml += `</ul>`;
      ...
      if (
        ...
        Object.keys(scores.missing_items).length > 0
      ) {
